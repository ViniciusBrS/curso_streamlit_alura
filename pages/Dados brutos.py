...
## TESTAR - FAZER ALGO DINAMICO

# with st.sidebar.expander('Categoria do produto'):
#     cat_produtos = st.multiselect('Selecione as categorias', dados['Categoria do Produto'].unique(), dados['Categoria do Produto'].unique())
...
#     preco = st.slider('Selecione o frete', 0, int(dados['Frete'].max()), (0, int(dados['Frete'].max())))
...
dados_filtrados = dados.query(strquery)
...
with col1:
    st.download_button('Baixar tabela em csv', data=converte_csv(dados_filtrados), file_name= nome_arquivo, mime='text\csv', on_click=mensagem_sucesso)